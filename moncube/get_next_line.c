...
int			get_line1(int const fd, char ** line, Elem * Curr, int nbCharLine)
{
	while (1)
	{
		if ((Curr->FileNbCharRead == 0)
			|| (Curr->FilePosition == Curr->FileNbCharRead))
		{
			if ((Curr->FileNbCharRead = read(fd, Curr->buff, 1)) == 0)
				return (*line[0] == 0 ? 0 : 1);
			Curr->FilePosition = 0;
			realloc_str(line, nbCharLine, Curr->FileNbCharRead);
		}
...
int			get_next_line1(int const fd, char ** line)
{
	...
	*line = ft_memalloc(1 + 1);
	return (get_line1(fd, line, Curr,0));
}
